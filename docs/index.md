<div class="hero" markdown>
# Patricia Lo
</div>
---

## About Me

I write documentation that helps developers ship faster. My samples span cloud infrastructure and game development — two domains where clarity directly impacts whether engineers can adopt a system or get stuck.

I focus on the decisions behind the architecture, not just the steps to reproduce it.

[Reach out on LinkedIn](https://linkedin.com/in/patricialo) or [view my GitHub](https://github.com/pkwlo).

---

## Writing Samples

<div class="sample-grid" markdown>

<div class="sample-card" markdown>

### [Serverless Backend Architecture Guide](aws-serverless-guide/index.md)

A comprehensive architecture and implementation guide for a production serverless backend built with AWS CDK, covering infrastructure-as-code patterns, DynamoDB data modeling, API Gateway configuration, Cognito authentication, and modular feature constructs.

<span class="tag">AWS</span>
<span class="tag">CDK</span>
<span class="tag">Serverless</span>

**Format:** Long-form architecture documentation

**Audience:** Backend engineers, cloud architects, and DevOps practitioners

</div>

<div class="sample-card" markdown>

### [Unity Character Movement Tutorial](unity-movement-tutorial/index.md)

A step-by-step tutorial walking developers through implementing a player movement system using Unity's Character Controller, covering input handling, frame-rate-independent motion, jumping mechanics, and camera integration.

<span class="tag">Unity</span>
<span class="tag">C#</span>
<span class="tag">Game Dev</span>

**Format:** Step-by-step tutorial with code examples

**Audience:** Intermediate developers with basic C# and Unity experience

</div>

</div>

---

## What I Bring

<div class="skills-grid" markdown>

<div class="skill-item" markdown>
**Developer Documentation**
API references, architecture guides, and onboarding docs for engineering teams.
</div>

<div class="skill-item" markdown>
**Tutorials & How-To Guides**
Step-by-step instructional content with code examples and diagrams.
</div>

<div class="skill-item" markdown>
**Technical Depth**
Hands-on experience with AWS, TypeScript, Python, Java, and real-time systems.
</div>

</div>
